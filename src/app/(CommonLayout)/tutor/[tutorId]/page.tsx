...
import { BookOpen, Briefcase, Clock, Star } from "lucide-react";
  ...
  if (!tutor) {
    return (
      <div className="container mx-auto py-20 text-center">
        ...
        <Link
          href="/courses"
          className="text-violet-600 mt-4 inline-block font-bold"
        >
          ← Browse Courses
        </Link>
      </div>
  ...
  return (
    <div className="container mx-auto py-12 px-4 max-w-5xl">
      ...
      <Card className="rounded-[3rem] border-none shadow-xl mb-10 overflow-hidden">
        <div className="h-32 bg-linear-to-r from-violet-600 to-indigo-600" />
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            <div className="w-28 h-28 rounded-3xl bg-white shadow-xl flex items-center justify-center text-4xl font-black text-violet-600 border-4 border-white">
              {tutor.user?.avatar ? (
                <img
                  src={
                    tutor.user.avatar.startsWith("http")
                      ? tutor.user.avatar
                      : `${process.env.NEXT_PUBLIC_BASE_URL?.replace("/api/v1", "")}${tutor.user.avatar}`
                  }
                  alt={tutor.user.name}
            ...
            <div className="flex-1 pt-10 md:pt-0 mt-4">
              ...
              <div className="flex flex-wrap gap-4 mt-4 text-sm text-slate-600">
                <span className="flex items-center gap-1">
                  ...
                  {tutor.avgRating ?? 0} avg rating
                </span>
                <span className="flex items-center gap-1">
                  <BookOpen className="w-4 h-4" />
                  {tutor.courses?.length ?? 0} courses
                </span>
                <span className="flex items-center gap-1">
                ...
                <span className="flex items-center gap-1">
                  <Briefcase className="w-4 h-4" />${tutor.hourlyRate}/hr
                </span>
              </div>
            </div>
          </div>
          {tutor.bio && (
            <p className="mt-6 text-slate-600 leading-relaxed border-t pt-6">
              {tutor.bio}
            </p>
          )}
        </CardContent>
              ...
              <Link href={`/courses/${course.id}`} key={course.id}>
                <Card className="rounded-[2rem] border-none shadow-md hover:shadow-xl transition-all cursor-pointer group mb-4">
                  <CardContent className="p-6 flex items-center justify-between">
                    <div>
                      <h3 className="font-black text-slate-800 group-hover:text-violet-600 transition-colors">
                        {course.title}
                      </h3>
        ...
        <div>
          ...
          {tutor.reviews?.length > 0 ? (
            tutor.reviews.slice(0, 5).map((review: any) => (
              <Card
                key={review.id}
                className="rounded-[1.5rem] border-none shadow-sm mb-4"
              >
